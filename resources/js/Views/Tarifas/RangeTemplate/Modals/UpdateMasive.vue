	...
	export default {
        ...
        computed:{
            setItems(){
                return this.items.map((item) =>({
                    name: `${item.relationships.roomType.attributes.name} - ${item.attributes.date_start} - ${item.attributes.date_end}  (${item.relationships.partialRate.attributes.name}-${item.attributes.rate})`,
                    id: item.id,
                }));
            },
            setRoomTypes() {
		...
		methods: {
			...
			createPermission: function () {
				var url = "/tarifas/RangeTemplate/masive-update";
